...
static imgsensor_info_struct imgsensor_info = {
	...
	/*zhengjiang.zhu@prize.Camera.Driver  2018/11/29  modify checksum  for ata test*/
	.checksum_value =0xe0bcdc67,//0xc15d2913,
...
{{ 4208, 3120,	  24,	0, 4160, 3120, 2080,  1560, 0000, 0000, 2080,  1560,	  0,	0, 2080, 1560}, // Preview 
 ...
 { 4208, 3120,	  24,   0, 4160, 3120, 2104,  1560, 0000,  376, 2104,  1184,	  0,	0, 2104, 1184}, //hight speed video
 { 4208, 3120,	  24,   0, 4160, 3120, 2104,  1560, 0000,  376, 2104,  1184,	  0,	0, 2104, 1184}};// slim video
...
{
    kal_uint16 reg_gain;
    //prize fengshangdong modify at 20190115
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor(0x0221,0x11);
	write_cmos_sensor(0x0222,0x01);
	write_cmos_sensor(0x0340,0x04);// 05
	write_cmos_sensor(0x0341,0xC8);//08
	write_cmos_sensor(0x0342,0x13);
	write_cmos_sensor(0x0343,0x90);
	write_cmos_sensor(0x0344,0x00);
	write_cmos_sensor(0x0345,0x00);
	...
	write_cmos_sensor(0x0305,0x03);
	write_cmos_sensor(0x0306,0x00);
	write_cmos_sensor(0x0307,0x74);//79
	write_cmos_sensor(0x0309,0x0A);
	write_cmos_sensor(0x030B,0x01);
	write_cmos_sensor(0x0310,0x00);

	write_cmos_sensor(0x0820,0x0E);//0F
	write_cmos_sensor(0x0821,0x80);// 20
	write_cmos_sensor(0x0822,0x00);
	write_cmos_sensor(0x0823,0x00);
	write_cmos_sensor(0x3A03,0x06);
	write_cmos_sensor(0x3A04,0x68);
	...
	write_cmos_sensor(0x3011,0x00);
	write_cmos_sensor(0x3013,0x00);
	write_cmos_sensor(0x0202,0x04);//04
	write_cmos_sensor(0x0203,0xBE);//FE
	write_cmos_sensor(0x0224,0x01);
	write_cmos_sensor(0x0225,0xF4);
	write_cmos_sensor(0x0204,0x00);
	write_cmos_sensor(0x0205,0x00);
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
    ...
    static SENSOR_VC_INFO_STRUCT *pvcinfo;
	...
	switch (feature_id) {
		...
		break;  // /*zhengjiang.zhu@prize.Camera.Driver  2018/12/25  add for ae is unstable*/
...
//kin0603
UINT32 IMX214_MIPI_RAW_SensorInit(SENSOR_FUNCTION_STRUCT **pfFunc)
